...
export default function Pago() {
  ...
  const cantidadRecibida = params.cantidadPizzas ? Number(params.cantidadPizzas) : 1;

  const lat = params.lat ? Number(params.lat) : null;
  const lng = params.lng ? Number(params.lng) : null;

  const [productos, setProductos] = useState([
    { id: 1, nombre: 'Pizza', precioUnitario: 12.99, cantidad: cantidadRecibida, imagen: require('@/assets/images/Pizza.jpg') },
  ]);
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
          ...
          {/* AQUÍ ESTÁ EL CAMBIO: Se indica que el mapa debe regresar a esta pantalla específica de pizzas */}
          <TouchableOpacity 
            style={styles.address} 
            onPress={() => router.push({
              pathname: "/mapa",
              params: { returnScreen: "/PagoPizzas" } // Ajusta esta ruta según el nombre exacto de tu archivo de pago de pizzas
            })}
          >
