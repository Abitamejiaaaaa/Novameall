...
export default function Pago() {
  ...
  const cantidadRecibida = params.cantidadTosta ? Number(params.cantidadTosta) : 1;

  const lat = params.lat ? Number(params.lat) : null;
  const lng = params.lng ? Number(params.lng) : null;

  const [productos, setProductos] = useState([
    { id: 1, nombre: 'Toasts', precioUnitario: 2.25, cantidad: cantidadRecibida, imagen: require('@/assets/images/TOSTADAS.jpg') },
  ]);
    ...
    <SafeAreaView style={styles.container}>
      <View style={styles.content}>
        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.scroll}>
          ...
          {/* AQUÍ ESTÁ EL CAMBIO: Se le indica que el mapa debe regresar a esta pantalla de pago específica */}
          <TouchableOpacity 
            style={styles.address} 
            onPress={() => router.push({
              pathname: "/mapa",
              params: { returnScreen: "/PagoTostadas" } // Ajusta esta ruta según el nombre exacto de este archivo en tu app
            })}
          >
